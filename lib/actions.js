...
export async function updateAthlete(athleteId, updates) {
  ...
  if (!name) return { ok: false, error: "Name is required" };
  if (updates.track && !["9th", "10th-roadmap", "11th-retainer"].includes(updates.track)) {
    return { ok: false, error: "Invalid track" };
  }
  await mutateJsonFile(
    "data/roster.json",
    (data) => {
      ...
      if (!athlete) return;
      athlete.name = name;
      if (updates.track) athlete.track = updates.track;
      athlete.classOf = updates.classOf ? Number(updates.classOf) : null;
      athlete.parents = (updates.parents || [])
        .filter((p) => p && p.trim())
        .map((n) => ({ name: n.trim() }));
      athlete.mentorName = updates.mentorName?.trim() || null;
      athlete.mentorEmail = updates.mentorEmail?.trim() || null;
      athlete.athleteEmail = updates.athleteEmail?.trim() || null;
      athlete.parentEmails = (updates.parentEmails || [])
        .filter((e) => e && e.trim())
        .map((e) => e.trim());
      athlete.calendarKeywords = (updates.calendarKeywords || [])
        .filter((k) => k && k.trim())
        .map((k) => k.trim());
      athlete.driveFolderUrl = updates.driveFolderUrl?.trim() || null;
    },
...
export async function updateDeliverable(athleteId, deliverableId, { label, due }) {
  const trimmedLabel = (label || "").trim();
  if (!trimmedLabel) return { ok: false, error: "Label required" };
  if (!due) return { ok: false, error: "Due date required" };
  await mutateJsonFile(
    ...
    (data) => {
      ...
      const d = athlete?.deliverables?.find((x) => x.id === deliverableId);
      if (!d) return;
      d.label = trimmedLabel;
      d.due = due;
    },
    `Update deliverable ${deliverableId}`
  );
  revalidatePath("/");
  return { ok: true };
}
